...
void lcd_set_rst(uint8_t state)
{
...
void lcd_set_dc(uint8_t state)
{
    gpio_set_level(lcd_obj->pin_dc, state);
}

void lcd_set_cs(uint8_t state)
{
    if (lcd_obj->pin_cs < 0) {
        return;
    }
...
void lcd_set_blk(uint8_t state)
{
    if (lcd_obj->pin_bk < 0) {
        return;
    }
...
static void IRAM_ATTR lcd_isr(void *arg)
{
    ...
    if (int_st.out_eof) {
        xQueueSendFromISR(lcd_obj->event_queue, (const void *)&int_st.val, &HPTaskAwoken);
    }
...
static void i2s_write_data(uint8_t *data, size_t len)
{
    ...
    // 处理完整一段数据， 乒乓操作
    for (x = 0; x < cnt; x++) {
        memcpy((void *)lcd_obj->dma[(x % 2) * lcd_obj->half_node_cnt].buf, data, lcd_obj->half_buffer_size);
    ...
    // 处理剩余非完整段数据
    if (cnt) {
        memcpy((void *)lcd_obj->dma[(x % 2) * lcd_obj->half_node_cnt].buf, data, cnt);
...
static void lcd_config(lcd_config_t *config)
{
   ...
   // 配置时钟
    ...
    esp_intr_alloc(ETS_I2S0_INTR_SOURCE, ESP_INTR_FLAG_LOWMED | ESP_INTR_FLAG_IRAM, lcd_isr, NULL, NULL);
}
...
void lcd_dma_config(lcd_config_t *config) 
{
    ...
    if (config->max_buffer_size >= LCD_DMA_MAX_SIZE * 2) {
        lcd_obj->dma_size = (LCD_DMA_MAX_SIZE / (config->bit_width >> 3)) * (config->bit_width >> 3);
...
int lcd_init(void)
{
    ...
    ESP_LOGI(TAG, "LCD initialize done");
