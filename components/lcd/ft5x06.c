...
static uint16_t read_word(uint8_t reg)
{
    uint8_t data[2];
    uint16_t result;
    ft5x06_read(reg, 2, (uint8_t *)&data);
...
static esp_err_t i2c_master_init(uint8_t _i2c_num)
{
    i2c_config_t conf;
    conf.mode = I2C_MODE_MASTER;
    conf.sda_io_num = FT5x06_I2C_SDA;
    conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
    conf.scl_io_num = FT5x06_I2C_SCK;
    conf.scl_pullup_en = GPIO_PULLUP_ENABLE;
    conf.master.clk_speed = 400000;
    i2c_param_config(_i2c_num, &conf);
    return i2c_driver_install(_i2c_num, conf.mode, 0, 0, 0);
}
...
void ft5x06_init()
{
    ESP_ERROR_CHECK(i2c_master_init(FT5x06_I2C_BUS));
    // ets_delay_us(1000);
